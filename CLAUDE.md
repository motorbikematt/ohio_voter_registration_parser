...
### CITY column data gap
The `CITY` column is entirely blank for 19 counties in the SWVF (Cuyahoga, Holmes, Sandusky, and 16 others — confirmed via parquet scan 2026-05-13). Most remaining counties are 40–95% blank (unincorporated voters with no CITY value). City aggregations in `jurisdictional_groupings.py` silently skip these voters. Cleveland and all other Cuyahoga cities are absent from `docs/data/city/` for this reason. Resolution path: join precinct → municipality from Ohio SOS precinct canvass files (P3 task).
...
- **Dashboard**: GitHub Pages `/docs` — county + precinct scope, manifest-driven, Chart.js. Live: https://motorbikematt.github.io/ohio_voter_registration_parser/
  ...
  - **Pass B complete (2026-05-12)**: Jurisdictions tab added. All 12 jurisdiction types routable via `?geo=jurisdiction&jurType=X&jurCounty=Y&jurName=Z`. Cascade type→county→name selectors; county picker shown only for county-scoped types. Legacy `geography:"city"` sections in `manifest.json` retained with comment flagging future removal; they reference old table-format JSON, not the 6-chart bundle.
  - **Pass C complete (2026-05-13)**: UI consolidated. Dead geo sub-filter buttons removed (City Precincts, Congress — no data). "County" sub-filter renamed to "Analysis". City scope tab aliased to Jurisdictions > Cities (full 6-chart bundle; legacy city table route retired). Main county-select hidden in Jurisdictions scope. IntersectionObserver hash scrollspy added. `docs/data/city/index.json` extended with 19 `available:false` gap entries for counties with blank SWVF CITY column (Cuyahoga, Holmes, Sandusky, etc.). Gap entries render in a separate `<optgroup>` in the dropdown; selecting one shows an amber unavailability panel with voter count and source explanation. Persistent collapsible info note shown whenever type=Cities. CSS added for both panels with full dark-mode support (`docs/assets/style.css`).
- **Dashboard JSON status**: County/precinct current as of 2026-05-08. Per-type `docs/data/{type}/index.json` files built 2026-05-12 for all 12 types (1272 townships, 683 villages, 366 local school districts, 212 cities, etc.). Townships/villages/municipal_court_districts require re-run of `python jurisdictional_groupings.py --jurisdictions townships,villages,municipal_court_districts` after deleting old phantom-merged dirs (`docs/data/township`, `docs/data/village`, `docs/data/municipal_court_district`) — county-scoped slug format changed in Pass A.
...
- **Enriched-parquet cache (Pass B, 2026-05-12)**: persistent cache at `source/parquet_enriched/enriched_voters.parquet` skips the ~80s `clean_voter_data()` cost on subsequent option 4 runs. Freshness check compares cache mtime against `max(latest_raw_partition_mtime, classifier_src_mtime)` — catches cohort taxonomy changes as well as new raw data. Atomic write via `.parquet.tmp` → `.replace()`. Implemented in both `voter_data_cleaner_v2.py` and `jurisdictional_groupings.py`.
- Install: `.venv\Scripts\pip install orjson psutil`

## Git

- `.git/index.lock` from failed sandbox git ops: `Remove-Item ".git\index.lock" -Force`, then commit from PowerShell or VS Code.
