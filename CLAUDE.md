...
*Last updated: 2026-05-12 (Pass B: enriched-parquet cache in `voter_data_cleaner_v2.py` and `jurisdictional_groupings.py`; Jurisdictions tab in dashboard with all 12 types; `jurisdictionScopes` registry in `manifest.json`; per-type `docs/data/{type}/index.json` files; `export_jurisdiction_index()` in `jurisdictional_groupings.py`; pytest suite `tests/test_pass_b_*.py` — 171 passed, 21 skipped.)*
