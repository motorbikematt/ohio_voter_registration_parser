...
- **Working/intermediate files**: `D:\vibe\election-data\`
- **Final deliverables**: `D:\vibe\election-data\`
...
- **Active scripts**:
  ...
  - `voter_data_cleaner_v2.py` — core engine (~3,600 lines); includes `classify_all_voters_primary_history()` universal cohort classifier; `clean_voter_data()` auto-attaches 15 cohort columns to every row; `export_precinct_charts()` writes per-precinct JSON for all 6 chart types
...
- **Dashboard**: GitHub Pages at `docs/` — county + precinct scope tabs; manifest-driven; Chart.js
  - Live at: https://motorbikematt.github.io/ohio_voter_registration_parser/
  - Deep-link format: `?county=Montgomery&geo=precinct-detail&precinct=DAYTON+8-B#decade-distribution`
  - Deep-link bug fixed (2026-05-07): `_populatePrecinctDropdown` in `docs/charts.js` now calls `_filterSections` + `_renderVisibleSections` + `_rebuildNav` after injecting precinct-detail sections on initial URL load.
- **Dashboard JSON status**: STALE — `docs/data/` was built before `export_precinct_charts()` was patched to use 8-cohort COHORT_SLICES for the party doughnut. Must run full statewide build (pipeline option 1 or 2) to regenerate all JSON with consistent labels.
- **Cohort taxonomy** (2026-05-07): 12 cohorts → 8 cohort_family values. PURE_R/D = regist